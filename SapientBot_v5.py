...
# Set up Google Cloud DialogFlow (if applicable)
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "/path/to/your/service_account.json"
os.environ["TF_ENABLE_ONEDNN_OPTS"] = "0"

# Initialize Hugging Face model
tokenizer = BertTokenizer.from_pretrained("bert-base-uncased")  # or your custom model path
...
# Add your bot logic below this line
@client.event
async def on_ready():
    print(f"{client.user.name} has connected to Discord!")
# ============================
# Dialogue Flow API Setup
# ============================
# Set up Google Cloud DialogFlow (if applicable)
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "sapientbot-f56d520d5e39ey.json"
project_id = "sapientbot"
...
# Example greeting dictionary for user mentions
custom_greetings = {
    'example_user': 'Hello, example_user!'
}
# Function to handle DialogFlow interaction
...
# Single on_message event handler for all interactions
@bot.event
async def on_message(message):
    ...
    if message.content.startswith("!") and user_mode == "command":
        await bot.process_commands(message)  # Handle commands
        return

    # Handle non-command conversation in chat mode
    if user_mode == "chat":
        intent = recognize_intent(message.content)

        # Respond to recognized intents
        if intent == "greeting":
            ...
            # For unknown intents, use sentiment analysis or AI response
    ...
    # Ensure commands are processed when user is in command mode
